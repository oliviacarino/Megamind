...
# Session recording — accumulated raw voltage samples for .npy export
left_recording  = []
right_recording = []
...
def adc_thread():
    ...
    while state["running"]:
        ...
        if now >= next_time:
            ...
            # Accumulate raw voltage for .npy export
            left_recording.append(left_v)
            right_recording.append(right_v)
...
def save_recording():
    """Save raw voltage samples from this session as .npy files.
    Output is Float32 voltage at 250 SPS — same format as S001R04_adapted.npy
    and can be loaded directly into Megamind Simulation mode.
    """
    if not left_recording:
        print("[recording] No samples to save.")
        return

    ts = time.strftime("%Y%m%d_%H%M%S")
    left_path  = f"session_{ts}_left.npy"
    right_path = f"session_{ts}_right.npy"

    np.save(left_path,  np.array(left_recording,  dtype=np.float32))
    np.save(right_path, np.array(right_recording, dtype=np.float32))

    duration = len(left_recording) / FS
    print(f"[recording] Saved {len(left_recording)} samples ({duration:.1f}s @ {FS} SPS)")
    print(f"[recording]   Left  → {left_path}")
    print(f"[recording]   Right → {right_path}")
    print(f"[recording] Load in Megamind Sim mode: upload either .npy file")
